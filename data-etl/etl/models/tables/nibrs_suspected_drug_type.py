...
def get_headers() -> list[TableField]:
    return [
        TableField(
            name="SUSPECTED_DRUG_TYPE_ID",
            sql_type=SQLConfig.Types.INTEGER,
            is_primary=True,
        ),
        TableField(name="SUSPECTED_DRUG_CODE", sql_type=SQLConfig.Types.STRING),
        TableField(name="SUSPECTED_DRUG_NAME", sql_type=SQLConfig.Types.STRING),
    ]
